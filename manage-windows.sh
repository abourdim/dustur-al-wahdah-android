#!/usr/bin/env bash
# ============================================================================
#  Dustur al-Wahdah Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/20-dustur-al-wahdah}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.dusturalwahdah}"
AVD_NAME="${AVD_NAME:-dustur_al_wahdah_test}"
...
cmd_test() {
  ...
  local out="/tmp/dustur-al-wahdah-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Dustur al-Wahdah Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
